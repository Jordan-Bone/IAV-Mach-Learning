...
ggplot(cls3,aes(x=reorder(Subtype,-N),y=N,fill=ifelse(Class=="Aves",Class,Family)))+geom_col()+theme(legend.position = "bottom",axis.text.x = element_text(angle = 90))
...
for(i in 1:length(mods)){
  ...
  caretStack(c("two_mer"=tmer_mod,"CTDc"=ctdc_mod,"CTDd"=ctdd_mod,"CTDt"=ctdt_mod,"triad"=ctri_mod,"pseaac"=psea_mod),weights=ifelse(tmp_ft$Class =="Aves",
                                        (1/table(tmp_ft$Class)[1]) * 0.5,
                                        (1/table(tmp_ft$Class)[2]) * 0.5), preProc=c("center","scale"), metric = "ROC", method="glmnet") %>% saveRDS(paste0("Comp/Stack/",mods[i],".rds"))
}
