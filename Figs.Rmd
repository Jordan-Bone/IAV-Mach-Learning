...
readRDS("Phylogenies/PA_tree.rds") %>% ggtree(aes(colour=host))+ geom_nodelab(aes(label=node),hjust = -0.5)+ theme(legend.position = c(.05, .75))+labs(title = "PA Tree")
```
...
ggplot(subset(recNode,Protein=="HA"),aes(as.factor(Feat1),Feat2,fill=Rate))+geom_tile()+ scale_fill_gradient(low="grey90",high="forestgreen")+theme(legend.position = "none")
```
