...
subset(uid_ref,!is.na(Subtype)) %>% group_by(Subtype) %>% summarise(Num=length(Subtype)) %>%  ggplot(aes(x=reorder(Subtype,-Num),y=Num,fill=Subtype %in% held_subtypes))+ geom_bar(stat="identity")+ geom_label(aes(x=20,y=200,label="23 of which appear\nmore than 25 times"),fill="#53CCCD")+ geom_segment(aes(x=70,xend=59.5,y=310,yend=20),arrow = arrow(angle = 30, length = unit(0.2, "cm"), type = "closed"),colour="#53CCCD")+ 
  geom_label(aes(75,1100,label="Σ = 4568"),colour="black",fill="white",size=3)+
  geom_label(aes(50,430,label="Σ = 308"),colour="#F8766D",fill="white",size=3)+
  geom_label(aes(12,790,label="Σ = 4260"),colour="#53CCCD",fill="white",size=3)+
...
<span style="color:blue">Note that some viruses had multiple proteins</span>
...
```
<span style="color:blue">This is a confidence matrix of the model performance overall. </span>
...
m3 <- ggplot(classes,aes(Host,`Balanced Accuracy`,colour=Host))+geom_point(size=3)+ylim(0,1)+geom_hline(yintercept = 0.5,linetype="longdash",colour="grey")+theme(legend.position="none")
...
```
Aagin, not sure what this is showing that the previous graphs weren't. What is the different between pos pred and sens?

```{r}
ggplot(mc_byc,aes(Host,Precision,colour=Protein))+geom_jitter(height=0,width=0.2)
ggplot(mc_byc,aes(Host,F1,colour=Protein))+geom_jitter(height=0, width=0.2)
ggplot(mc_byc,aes(Host,Balanced.Accuracy,colour=Protein))+geom_jitter(height=0, width=0.2)
```
...
This is the variable importance stuff, explain better how it was done and what the AUC loss means
