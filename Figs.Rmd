...
overall_results[,c(2,1,3)] %>% pivot_wider(names_from=ModelType, values_from=Accuracy) %>% arrange(Protein) %>% kbl() %>% kable_styling(full_width = F) %>% collapse_rows(1)
```
...
for (PRT in c("PB2","PB1","PA","HA","NP","NA","M1","NS1")){
  msmtch <- read.csv(paste0("Phylogenies/iqtree/Annotations/",PRT,".txt"))
  mismatches <- bind_rows(mismatches,msmtch)
}

mismatches %>% group_by(PRT,Mismatch) %>% summarise(N=length(Mismatch)) %>% pivot_wider(names_from = Mismatch, values_from = N)
```
...
## BayesTraits
```{r}
multi_trs_rates <- results$Log$results %>% select(-c("Tree.No","No.Off.Parmeters","No.Off.Zero","Model.string")&-starts_with("Root")) %>% pivot_longer(cols = 3:32,names_to = "Transition",values_to = "Rate")

multi_trs_rates$state1 <- multi_trs_rates$Transition %>% str_replace_all("q","") %>% str_sub(1,1)
multi_trs_rates$state2 <- multi_trs_rates$Transition %>% str_replace_all("q","") %>% str_sub(2,2)
rate_sums <- multi_trs_rates %>% group_by(state1,state2) %>% summarise(Rate=mean(Rate))

ggplot(data=rate_sums,aes(state1,state2,fill=Rate))+
  geom_tile(colour="white")+geom_text(aes(label=round(Rate,2)))+scale_fill_gradient(low="white",high="forestgreen")+
  labs(x="Origin",y="Spillover",subtitle = "Transition Rates Between Hosts",title = "M1 protein")

# lrtest(results,results_2)
# bftest(results,results_2)
```
...
ggplot(subset(trait_labs,mRate>1),aes(Donor,Recip,fill=mRate))+geom_tile(colour="white")+ scale_fill_gradient(low="grey90",high="forestgreen")+ 
  geom_text(aes(label=paste0(round(mRate,2),"\n±",round(sdRate,2))))+
  facet_wrap(~Protein,ncol=4)+theme_bw()+theme(legend.position = "none")+labs(title = "Discrete Trait Transition Rates of IAV Proteins",subtitle = "Modelled by rjMCMC Multistate Estimation in BayesTraits")
...
```{r}
colz <- c("Avian" = "skyblue", "Mammalian" = "indianred")
...
```{r}
ggplot(transition_rates,aes(Iteration,Lh,colour=Protein))+geom_point()

ggplot(pb2_ace,aes(q01,q10))+geom_point()+
  geom_label(aes(35,8,label=paste("A -> M\n",round(mean(ace_df$q01),2))),fill="skyblue")+
  geom_label(aes(8,30,label=paste("M -> A\n",round(mean(ace_df$q10),2))),fill="indianred")+
  labs(caption = "Transition Rates from State 0 (Avian) to State 1 (Mammalian) and reverse.",subtitle = "Transition Rates Between States",title = "M1 protein")

ggplot(pb2_ace)+
  geom_point(aes(Iteration,RecNode.P.0.,colour="Avian"))+
  geom_point(aes(Iteration,RecNode.P.1.,colour="Mammalian"))+
  scale_colour_manual(name="Host",values=colz)+
  labs(x="MCMC iterations",y="Proportion of Node States")
```
...
Transition states between traits is meant to show the likelihood of roving from one host to another.
Similarly, the continuous trait reconstruction models show the difference in protein features along branches

Now where exactly do we expect to see big changes - where are the known host shifts?

EIV2 H3N8 first emerged in 1963:
PB2, ha, na, m and NS1 reassorted with np, PB1 and pa?
...
```{r}
ggplot(recNode,aes(Feat1,Feat2,fill=Rate))+geom_tile()+facet_wrap(~Protein,ncol=4)+ scale_fill_gradient(low="grey90",high="forestgreen")+theme(legend.position = "none")
```
