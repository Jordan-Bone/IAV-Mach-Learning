...
for(j in c(10:12)){
  ...
  predict_class_test <- Map(function(model, newdata)
    if (nrow(newdata)>0){predict(model, newdata=newdata, type="raw")},
    model = rf, newdata = validate) %>% unlist %>% as.factor
  ...
  # predict_prob_test <- predict(MOD, newdata=validate, type="prob")
  # bind_rows(predict_class_test,predict_prob_test)
  
  if(MOD_TYPE=="M"){
    matrix_test <- confusionMatrix(predict_class_test %>% droplevels,
                                   validate %>% pull(label) %>% as.factor %>% droplevels)
    
    matrix_one_vs_all <- vector("list", length(levels(MODtestM$trainingData$.outcome)))
    for (i in seq_along(matrix_one_vs_all)) {
      positive.class <- levels(MODtestM$trainingData$.outcome)[i]
